<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:90:"C:\wamp64\www\tutorAssign\public/../app/index\view\department_head_tutor\time_setting.html";i:1479481616;}*/ ?>
<html>
...
<body cz-shortcut-listen="true">
    <div id="warn-alert" class="alert alert-info" style="display: none; position:fixed; top:10px; left: 300px;right: 300px; margin:auto; border: 2px solid red; opacity: 1; z-index: 2">
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
    var warn_text = new Array("提示：输入框不能为空！",
        "提示：截至时间 不得早于 起始时间！");
    ...
    function validate() {
        ...
        var dateLegal_flag = checkData(form2to4);
        showWarnInfo(form1_flag,form2to4_flag,dateLegal_flag);
        return form1_flag && form2to4_flag && dateLegal_flag;
    }

    function validateForm(form, formNum) {
        var isLegal = true;
        var idSet = new Array();
    ...
    function displayWarnStyleNumber(idSet) {
        for (var i = 0; i < idSet.length; ++i) {
            $("#" + idSet[i]).parent().addClass("has-error");
        }
    }

    function showWarnInfo(form1_flag,form2to4_flag,dateLegal_flag){
        if(!form2to4_flag || !form1_flag){
            $("#warn-alert-text").text(warn_text[0]);
            $("#warn-alert").show(300);
        }else if(!dateLegal_flag){
            $("#warn-alert-text").text(warn_text[1]);
            $("#warn-alert").show(300);
        }
    }
    </script>
