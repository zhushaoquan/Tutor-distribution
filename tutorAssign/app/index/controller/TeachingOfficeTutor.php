...
class TeachingOfficeTutor extends BaseController {
	...
	//	var_dump($dep);
	  //  $list=Db::query("select t.workNumber tnum,t.name tname,s.serialNum snum,s.name sname from user_teacher t,tc_student s,tc_result r where t.workNumber=r.workNumber and s.sid=r.sid");
		$list=Db::table('user_teacher t,user_student s,tc_result r')->where('t.workNumber=r.workNumber and s.sid=r.sid')->where('s.department','=',$dep)->field('t.workNumber as tnum,t.name as tname,s.serialNum as snum,s.name as sname')->order('s.serialNum')->paginate(8);
